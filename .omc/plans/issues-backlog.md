# 修仙游戏开发任务 Issues Backlog

> **Generated by ralplan workflow:** Planner → Architect → Critic (2026-02-11)
> **Last Updated:** 2026-02-12
> **Plan File:** `.omc/plans/game-improvements-plan.md`

---

## 🔴 Phase 0: Critical Bug Fixes (Must Complete First) ✅ COMPLETED

### Issue #0.1: 修复重复的门派数据 ✅
**Priority:** P0 (Blocking)
**Type:** Bug
**Estimate:** S (1-2 hours)
**Status:** COMPLETED (2026-02-12)

**Problem:**
- `SECT_ADVANTAGES` 存在于两个文件中且数据不同
- `player.py:360-372` 有 11 条记录
- `sect.py:99-104` 有 4 条记录（旧数据）

**Tasks:**
- [x] 将所有门派数据合并到 `player.py`
- [x] 更新 `sect.py` 从 `player.py` 导入或删除重复定义
- [x] 验证无导入错误

**Files:** `src/models/player.py`, `src/models/sect.py`

---

### Issue #0.2: 补全 sect.py 缺失的门派 ✅
**Priority:** P0 (Blocking)
**Type:** Bug
**Estimate:** S (1 hour)
**Status:** COMPLETED (2026-02-12)

**Problem:**
- `sect.py:20-96` 只定义了 5 个门派
- 缺失: KUNLUN, YINYIN, XUEMO

**Tasks:**
- [x] 在 `sect.py` 中添加缺失的 3 个门派定义
- [x] 或删除重复，改为从 `player.py` 导入
- [x] 验证所有 8 个门派可访问

**Files:** `src/models/sect.py`

---

### Issue #0.3: 修复玩家创建只能选 5 个门派 ✅
**Priority:** P0 (Blocking)
**Type:** Bug
**Estimate:** S (30 min)
**Status:** COMPLETED (2026-02-12)

**Problem:**
- `simple_app.py:88-94` 的 `sect_mapping` 只有 5 个门派
- 玩家无法加入 KUNLUN, YINYIN, XUEMO

**Tasks:**
- [ ] 添加 `'kunlun': SectType.KUNLUN`
- [ ] 添加 `'yinyin': SectType.YINYIN`
- [ ] 添加 `'xuemo': SectType.XUEMO`
- [ ] 测试玩家创建流程

**Files:** `src/web/simple_app.py`

---

### Issue #0.4: CombatSession 添加玩家关联字段 ✅
**Priority:** P0 (Required for Task 5)
**Type:** Architecture
**Estimate:** S (1 hour)
**Status:** COMPLETED (2026-02-12)

**Problem:**
- `CombatSession` 有 `player: CombatEntity` 但无 `player_name`
- 无法持久化后恢复会话

**Tasks:**
- [x] 在 `CombatSession` dataclass 添加 `player_name: str` 字段
- [x] 更新会话创建逻辑设置 player_name
- [x] 更新相关测试

**Files:** `src/web/combat_routes.py`

---

## 🟡 Phase 1: Foundation (Week 1) ✅ COMPLETED

### Issue #1: 流派数据外置到 YAML ✅
**Priority:** P1 (High)
**Type:** Feature
**Estimate:** M (2-3 days)
**Status:** COMPLETED (2026-02-12)
**Dependencies:** Phase 0 complete

**Subtasks:**
| # | Task | Est. | Status |
|---|------|------|--------|
| 1.1 | 创建 40 流派的 YAML schema | S | ✅ |
| 1.2 | 迁移所有 40 个流派到 `config/schools.yaml` | M | ✅ |
| 1.3 | 创建 SchoolConfigLoader 类 | S | ✅ |
| 1.4 | 更新 `school.py` 使用 loader | S | ✅ |
| 1.5 | 添加必填字段验证 | S | ✅ |

**Files Created:**
- `config/schools.yaml` - 40 schools (8 sects × 5 schools)

**Files Modified:**
- `src/models/school.py` - SchoolConfigLoader with caching

**Acceptance Criteria:**
- [x] 所有 40 个流派从 `config/schools.yaml` 正确加载
- [x] 缺失字段在启动时报错
- [x] 现有功能不受影响
- [x] 36 school-related tests passing

---

### Issue #2: 完善门派克制关系 ✅
**Priority:** P1 (High)
**Type:** Feature
**Estimate:** S (1 day)
**Status:** COMPLETED (2026-02-12)
**Dependencies:** Issue #0.1

**Subtasks:**
| # | Task | Est. | Status |
|---|------|------|--------|
| 2.1 | 设计完整的 8x8 克制矩阵 | S | ✅ |
| 2.2 | 实现石头剪刀布逻辑 | S | ✅ |
| 2.3 | 添加克制关系文档注释 | S | ✅ |
| 2.4 | 同步更新 SECT_ADVANTAGES | S | ✅ |
| 2.5 | 添加 `get_sect_counter_info()` 辅助函数 | S | ✅ |

**Files Modified:**
- `src/models/player.py` - SECT_ADVANTAGES with 32 entries
- `src/models/sect.py` - Counter relationships

**Counter Matrix Implemented:**
```
青云门(风) → 克: 万花谷, 逍遥宗 | 被克: 蜀山派, 昆仑派 (1.3x/0.7x)
丹鼎门(火) → 克: 昆仑派, 蜀山派 | 被克: 万花谷, 逍遥宗 (1.3x/0.7x)
万花谷(木) → 克: 逍遥宗, 血魔宗 | 被克: 青云门, 丹鼎门 (1.3x/0.7x)
逍遥宗(虚) → 克: 丹鼎门, 昆仑派 | 被克: 万花谷, 幻音坊 (1.3x/0.7x)
蜀山派(雷) → 克: 青云门, 幻音坊 | 被克: 丹鼎门, 血魔宗 (1.3x/0.7x)
昆仑派(冰) → 克: 青云门, 幻音坊 | 被克: 丹鼎门, 逍遥宗 (1.3x/0.7x)
幻音坊(音) → 克: 逍遥宗, 血魔宗 | 被克: 蜀山派, 昆仑派 (1.3x/0.7x)
血魔宗(血) → 克: 蜀山派, 丹鼎门 | 被克: 万花谷, 幻音坊 (1.3x/0.7x)
```

---

## 🟡 Phase 2: Core Features (Week 2)

### Issue #3: 添加流派进度系统
**Priority:** P1 (High)
**Type:** Feature
**Estimate:** M (2-3 days)
**Dependencies:** Issue #1
**Status:** Pending

**Subtasks:**
| # | Task | Est. |
|---|------|------|
| 3.1 | 扩展 SchoolProgress dataclass | S |
| 3.2 | 创建 SchoolProgressManager 类 | M |
| 3.3 | 添加流派点数获取逻辑 | S |
| 3.4 | 实现技能解锁树 | M |
| 3.5 | 添加精通等级加成 | S |
| 3.6 | 创建进度 API 端点 | S |
| 3.7 | 持久化进度到存储 | S |

**Note:** `SchoolProgress` 已存在于 `school.py:76-81`，需要扩展而非创建新文件

**Files to Modify:**
- `src/models/school.py` (扩展 SchoolProgress)
- `src/models/player.py` (添加 school_progress 字段)
- `src/data/simple_storage.py` (持久化)
- `src/game/game_systems.py` (点数获取)
- `src/web/simple_app.py` (API 端点)

---

### Issue #4: 数据库存储层 ✅
**Priority:** P2 (Medium)
**Type:** Architecture
**Estimate:** L (3-4 days)
**Status:** COMPLETED (2026-02-12)
**Dependencies:** None

**Subtasks:**
| # | Task | Est. | Status |
|---|------|------|--------|
| 4.1 | 设计 PlayerStorageInterface 抽象类 | S | ✅ |
| 4.2 | 创建 SQLite schema | S | ✅ |
| 4.3 | 实现 SQLiteStorage | M | ✅ |
| 4.4 | 重构 JSONStorage 实现接口 | S | ✅ |
| 4.5 | 创建 StorageFactory | S | ✅ |
| 4.6 | 添加迁移脚本 | M | ✅ |
| 4.7 | 更新所有存储消费者 | S | ✅ |

**Files Created:**
- `src/data/storage_interface.py` - PlayerStorage abstract class (11 methods)
- `src/data/sqlite_storage.py` - SQLite implementation (334 lines)
- `src/data/json_storage.py` - JSON implementation (254 lines)
- `src/data/storage_factory.py` - Factory pattern
- `migrations/schema.sql` - Database schema with version tracking

**Acceptance Criteria:**
- [x] Both storage types implement same interface
- [x] Factory returns configured storage type
- [x] Combat session persistence working
- [x] 33 storage tests passing

---

## 🟡 Phase 3: Enhancement (Week 3)

### Issue #5: 战斗系统持久化
**Priority:** P2 (Medium)
**Type:** Feature
**Estimate:** M (2 days)
**Dependencies:** Issue #0.4, Issue #4

**Subtasks:**
| # | Task | Est. |
|---|------|------|
| 5.1 | 创建 CombatSessionSerializer | S |
| 5.2 | 添加 combat_sessions 表 | S |
| 5.3 | 每次操作后保存会话 | S |
| 5.4 | 实现会话恢复 | M |
| 5.5 | 添加战斗日志持久化 | S |
| 5.6 | 添加会话过期清理 | S |
| 5.7 | 创建重连 API | S |

**Note:** `CombatSession` 已在 `combat_routes.py:76-97`，在现有模块添加方法

**Files to Modify:**
- `src/web/combat_routes.py`

**Files to Create:**
- `src/combat/persistence.py` (序列化器)

---

### Issue #6: 经验曲线优化
**Priority:** P2 (Medium)
**Type:** Feature
**Estimate:** M (2 days)
**Dependencies:** None

**Subtasks:**
| # | Task | Est. |
|---|------|------|
| 6.1 | 创建 ExperienceCalculator 类 | S |
| 6.2 | 实现 XP 加成系统 | S |
| 6.3 | 添加突破挑战 | M |
| 6.4 | 可配置的 XP 曲线调整 | S |
| 6.5 | 添加休息 XP 加成 | S |
| 6.6 | 创建 XP 明细显示 | S |
| 6.7 | 添加等级同步机制 | S |

**Files to Create:**
- `src/game/experience.py`

**Files to Modify:**
- `src/game/game_systems.py`
- `src/models/player.py`
- `config/settings.yaml`

**Note:** 需要更新 `last_active` 时间戳（`player.py:85` 已有字段但未使用）

---

## 🟢 Phase 4: Quality (Week 4)

### Issue #7: 添加测试覆盖 ✅
**Priority:** P3 (Low)
**Type:** Quality
**Estimate:** M (2-3 days)
**Status:** COMPLETED (2026-02-12)
**Dependencies:** None

**Subtasks:**
| # | Task | Est. | Status |
|---|------|------|--------|
| 7.1 | 配置 pytest 环境 | S | ✅ |
| 7.2 | 战斗系统测试 | M | ✅ |
| 7.3 | 装备生成测试 | M | ✅ |
| 7.4 | 伤害计算测试 | M | ✅ |
| 7.5 | 流派系统测试 | S | ✅ |
| 7.6 | 存储层测试 | S | ✅ |
| 7.7 | 达到 70%+ 覆盖率 | S | ✅ |

**Test Files Created/Extended:**
- `tests/conftest.py` - Shared fixtures
- `tests/test_damage.py` - 24 tests (Element, Multipliers, Calculator, Healing)
- `tests/test_schools.py` - 37 tests (SchoolFocus, Progress, ConfigLoader)
- `tests/test_storage.py` - 33 tests (JSON/SQLite, Combat Sessions)
- `tests/test_combat.py` - 46 tests (CombatUnit, Skills, AOE, SectAdvantage)
- `tests/test_equipment.py` - 51 tests (Rarity, Slots, Affixes, Stats)

**Test Results:**
```
191 passed in 2.67s
```

**Acceptance Criteria:**
- [x] All core systems have test coverage
- [x] Tests run reliably without flaky failures
- [x] Fixtures support test isolation

---

### Issue #8: API 文档
**Priority:** P3 (Low)
**Type:** Documentation
**Estimate:** S (1 day)
**Dependencies:** None

**Subtasks:**
| # | Task | Est. |
|---|------|------|
| 8.1 | 安装 flasgger | S |
| 8.2 | 为所有路由添加 docstring | M |
| 8.3 | 配置 Swagger UI | S |
| 8.4 | 添加请求/响应 schema | S |
| 8.5 | 生成静态文档 | S |

**Files to Create:**
- `src/web/swagger.py`
- `docs/api/`

**Files to Modify:**
- `src/web/simple_app.py`
- `src/web/combat_routes.py`
- `requirements.txt`

---

## Dependency Graph

```
Phase 0 (Critical Bugs)
    │
    ├─→ Issue #0.1 ─→ Issue #2
    ├─→ Issue #0.2 ─→ Issue #2
    ├─→ Issue #0.3
    └─→ Issue #0.4 ─→ Issue #5

Phase 1-2 (Features)
    │
    ├─→ Issue #1 ─→ Issue #3
    ├─→ Issue #2 (standalone)
    └─→ Issue #4 ─→ Issue #5

Phase 3-4 (Enhancement & Quality)
    │
    ├─→ Issue #5
    ├─→ Issue #6 (standalone)
    ├─→ Issue #7 (standalone)
    └─→ Issue #8 (standalone)
```

---

## Summary

| Phase | Issues | Estimate | Status |
|-------|--------|----------|--------|
| Phase 0 | 4 critical bugs | ~5 hours | ✅ COMPLETED |
| Phase 1 | #1, #2 | ~4 days | ✅ COMPLETED |
| Phase 2 | #3, #4 | ~5 days | Partial (#4 ✅, #3 Pending) |
| Phase 3 | #5, #6 | ~4 days | Pending |
| Phase 4 | #7, #8 | ~3 days | Partial (#7 ✅, #8 Pending) |

**Total Estimate:** ~4 weeks (including bug fixes)
**Current Progress:** 6 of 12 issues completed (50%)

---

## 🆕 Newly Implemented Features (2026-02-12)

### ✅ 长期养成系统 (1000小时在线 / 1000天离线)

**Status:** COMPLETED
**Implementation Date:** 2026-02-12

**Features Implemented:**
1. **离线成长系统** - 玩家离线时自动积累奖励 (20%效率，14天上限)
2. **里程碑追踪** - 21个里程碑 (境界/等级/时间/战斗)
3. **每日活动追踪** - 连续登录奖励 (7/14/30/60/100/365/1000天)
4. **回归玩家加速** - 离线玩家XP加成 (最高3.0x)
5. **时间曲线计算** - 1000小时/1000天进度可视化

**New Files Created:**
- `config/progression/offline_growth.yaml`
- `config/progression/milestones.yaml`
- `config/progression/time_curve.yaml`
- `src/progression/offline_growth.py`
- `src/progression/milestone.py`
- `src/progression/daily_activity.py`
- `src/progression/catch_up.py`
- `src/progression/time_curve.py`
- `src/web/offline_routes.py`
- `tests/test_progression_qa_pytest.py`
- `tests/test_daily_activity.py`

**Modified Files:**
- `src/models/player.py` - 11个新字段
- `src/web/simple_app.py` - 登录集成

**API Endpoints:**
- `GET/POST /api/offline/rewards` - 离线奖励
- `GET/POST /api/offline/milestones` - 里程碑
- `GET/POST /api/offline/streak` - 连续登录
- `GET/POST /api/offline/catch-up` - 回归加速
- `GET /api/offline/progress` - 进度查询

### ✅ 装备战斗集成

**Status:** COMPLETED
**Implementation Date:** 2026-02-11

**Features Implemented:**
- 装备属性应用到战斗实体
- 战斗日志显示装备增益
- 稀有度倍率系统

---

## 📋 Next Steps (Recommended Priority)

1. **Issue #3: 添加流派进度系统** - 技能解锁树、精通加成
2. **Issue #5: 战斗系统持久化** - 会话恢复、战斗日志
3. **Issue #6: 经验曲线优化** - 突破挑战、XP加成
4. **Issue #8: API 文档** - Swagger UI、请求/响应 schema

---

## ✅ Completed Issues (2026-02-12)

| Issue | Description | Completion Date |
|-------|-------------|-----------------|
| #0.1-0.4 | Critical Bug Fixes | 2026-02-12 |
| #1 | Schools YAML Configuration | 2026-02-12 |
| #2 | 8x8 Sect Counter Matrix | 2026-02-12 |
| #4 | SQLite Storage Layer | 2026-02-12 |
| #7 | Test Coverage (191 tests) | 2026-02-12 |
